...
> {
  ...
  return useMutation({
    mutationFn: async (input: Omit<Trip, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
      logger.debug('Creating trip', { name: input.name, destination: input.destination });
      const result = await createTrip(input);
      return result;
    },
    onSuccess: (newTrip) => {
      logger.info('Trip created successfully', { id: newTrip.id, name: newTrip.name });
...
> {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<Trip> }) => {
      logger.debug('Updating trip', { id, updates: Object.keys(updates) });
      const result = await updateTrip(id, updates);
      return result;
    },
    onSuccess: (updatedTrip) => {
      logger.info('Trip updated successfully', { id: updatedTrip.id });
    ...
    onError: (error: Error) => {
      logger.error('Failed to update trip', { error: error.message });
    },
  });
}
...
export function useDeleteTrip(): UseMutationResult<void, Error, string> {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      logger.info('Trip deleted successfully', { id: deletedId });
    ...
    onError: (error: Error) => {
      logger.error('Failed to delete trip', { error: error.message });
    },
  });
}
...
> {
  ...
  return useMutation({
    mutationFn: async (input: Omit<TripDay, 'id' | 'created_at'>) => {
      logger.debug('Creating trip day', { tripId: input.trip_id, date: input.date });
      const result = await createTripDay(input);
      return result;
    },
    onSuccess: (newTripDay) => {
      logger.info('Trip day created successfully', { id: newTripDay.id, tripId: newTripDay.trip_id });
    ...
    onError: (error: Error) => {
      logger.error('Failed to create trip day', { error: error.message });
    },
  });
}
...
> {
  ...
  return useMutation({
    mutationFn: async (input: Omit<TravelDocument, 'id' | 'user_id' | 'created_at'>) => {
      logger.debug('Creating travel document', { type: input.type, tripId: input.trip_id });
      const result = await createTravelDocument(input);
      return result;
    },
    onSuccess: (newDocument) => {
      logger.info('Travel document created successfully', { id: newDocument.id });

      // Invalidate all documents lists
      void queryClient.invalidateQueries({ queryKey: queryKeys.travel.documents.all() });
    },
    onError: (error: Error) => {
      logger.error('Failed to create travel document', { error: error.message });
    },
  });
}
...
> {
  ...
  return useMutation({
    mutationFn: async (input: Omit<PackingList, 'id' | 'user_id' | 'created_at' | 'items'>) => {
      logger.debug('Creating packing list', { name: input.name, tripId: input.trip_id });
      const result = await createPackingList(input);
      return result;
    },
    onSuccess: (newList) => {
      logger.info('Packing list created successfully', { id: newList.id });

      // Invalidate all packing lists
      void queryClient.invalidateQueries({ queryKey: queryKeys.travel.packingLists.all() });
    },
    onError: (error: Error) => {
      logger.error('Failed to create packing list', { error: error.message });
    },
  });
}
