...
import type { DreamDraft } from '../../types/drafts';
import type { DreamCategory, DreamPriority, DreamStatus } from '../../types/lifeGoals';
import { DREAM_CATEGORIES, DREAM_PRIORITIES, DREAM_STATUSES } from '../../constants';

interface DreamFormProps {
  dreamDraft: DreamDraft;
  onDraftChange: (updates: Partial<DreamDraft>) => void;
  onSubmit: (event: FormEvent<HTMLFormElement>) => void;
  onCancel: () => void;
}

/**
 * Form for capturing a new dream
 */
export function DreamForm({ dreamDraft, onDraftChange, onSubmit, onCancel }: DreamFormProps): React.ReactElement {
  return (
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(event) => onDraftChange({ category: event.target.value as DreamCategory })}
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
          >
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(event) => onDraftChange({ priority: event.target.value as DreamPriority })}
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
          >
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            ...
            onChange={(event) => onDraftChange({ status: event.target.value as DreamStatus })}
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
          >
        ...
        <label className="flex flex-col gap-1 text-sm">
          ...
          <input
            ...
            onChange={(event) => onDraftChange({ estimatedCost: event.target.value })}
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
            placeholder="5000"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm">
          ...
          <input
            ...
            onChange={(event) => onDraftChange({ estimatedTimeframe: event.target.value })}
            className="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
