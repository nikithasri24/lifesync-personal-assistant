...
vi.mock('../../todos/hooks', () => ({
  useTodosDragDrop: ({ updateTaskMutation }: any) => {
    const handleDropOnSection = (sectionKey: string, _event: React.DragEvent) => {
      // Simulate what the real hook does: update due_date when dropped on calendar-date section
      if (sectionKey.startsWith('calendar-date-')) {
        const datePart = sectionKey.replace('calendar-date-', '')
        updateTaskMutation.mutate({ id: 't1', updates: { due_date: `${datePart}T00:00:00.000Z` } })
      }
    }
...
describe('Calendar date drop', () => {
  ...
  it('renders the Todos page with tasks', async () => {
  ...
  it('sets due_date when dropped on calendar-date section via drag handler', async () => {
    const mod = await import('../Todos')
    const Todos = mod.default
    const Wrapper = createWrapper()
    render(<Wrapper><Todos /></Wrapper>)
    ...
    // Invoke the handleDropOnSection directly with a calendar-date key
    expect(capturedHandleDropOnSection).not.toBeNull()
    capturedHandleDropOnSection!('calendar-date-2025-04-15', {} as React.DragEvent)

    expect(updateTaskMock).toHaveBeenCalled()
    const callArgs = updateTaskMock.mock.calls[0][0]
    expect(callArgs.updates.due_date).toContain('2025-04-15')
  })
})
