...
export default function Todos() {
  // Use local store as primary source with API fallback
  const { todos, addTodo, updateTodo, deleteTodo, addSubtask: addSubtaskToStore } = useAppStore();
  
  // Try to use API tasks for Calendar integration, but gracefully handle failures
  const { 
    tasks: apiTasks,
  ...
  const quickAddTask = async () => {
    ...
    if (!apiError && createApiTask) {
      try {
        ...
        console.warn('API sync failed, but task was saved locally:', err);
      }
    }
  ...
  const toggleTaskStatus = async (taskId: string) => {
    ...
    if (task) {
      ...
      if (!apiError && updateApiTask) {
        try {
          await updateApiTask(taskId, { 
            status: task.completed ? 'todo' : 'done'
          });
        } catch (err) {
          console.warn('API sync failed, but status was updated locally:', err);
        }
      }
    }
  };
        ...
        if (!apiError && updateApiTask) {
          try {
            await updateApiTask(taskId, {
              title: parsed.title,
              priority: parsed.priority as any,
              due_date: parsed.dueDate ? parsed.dueDate.toISOString() : currentTodo.dueDate?.toISOString() || null,
              project_id: parsed.projectId || currentTodo.projectId,
              tags: parsed.tags.length > 0 ? parsed.tags : currentTodo.tags
            });
          } catch (err) {
            console.warn('API sync failed, but task was updated locally:', err);
          }
        }
      }
    }
  ...
  const addSubtask = async (parentId: string) => {
    ...
    if (!apiError && createApiTask) {
      try {
        ...
        console.warn('API sync failed, but subtask was saved locally:', err);
      }
    }
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2" title={`Last checked: ${apiHealth.lastChecked?.toLocaleTimeString() || 'Never'}`}>
                <div className={`w-2 h-2 rounded-full ${
                  !apiHealth.isOnline ? 'bg-red-500' : apiLoading ? 'bg-yellow-500' : 'bg-green-500'
                }`}></div>
                <span className={`text-xs font-medium ${
                  !apiHealth.isOnline ? 'text-red-600' : apiLoading ? 'text-yellow-600' : 'text-green-600'
                }`}>
                  {!apiHealth.isOnline 
                    ? 'API Offline' 
                    : apiLoading 
              ...
              if (currentView === 'today') tasksToShow = applyFilters(getTodayTasks());
                ...
                return (
                  <div className="p-6">
                    ...
                    {/* Calendar Grid - Same as Calendar.tsx */}
                    {calendarView === 'month' && (
                        ...
                        <div className="flex-1 flex flex-col">
                          {(() => {
                            ...
                            return weeks.map((week, weekIndex) => (
                              <div key={weekIndex} className="relative flex h-32">
