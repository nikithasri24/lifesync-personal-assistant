...
const SUBTASK_STATUSES: Array<{ value: TaskStatus; label: string }> = [
...
const getSubtaskStatusStyles = (status: TaskStatus) => {
  switch (status) {
    case 'done':
...
export default function TodosWorkingFollowUp() {
  ...
  const convertApiTaskToTask = (apiTask: TaskData): Task => {
    return {
      ...
      tags: apiTask.tags || [],
      deleted: Boolean(apiTask.deleted),
    ...
    if (pomodoroTimer.isActive && pomodoroTimer.timeLeft > 0) {
      ...
      if (!pomodoroTimer.isBreak) {
        ...
        if (pomodoroTimer.taskId) {
          const task = getTaskById(pomodoroTimer.taskId);
          if (task) {
      ...
      (async () => {
        try {
          if (overId === 'sidebar-waiting') {
            ...
            // Task moved to calendar - no action needed
  ...
  const updateSubtaskStatus = async (subtaskId: string, status: TaskStatus) => {
    try {
      await apiUpdateTask(subtaskId, {
        status,
  ...
  const saveSubtaskEdit = async () => {
    ...
    const trimmedTitle = editingSubtask.title.trim();
    if (!trimmedTitle) {
      cancelSubtaskEdit();
      return;
    }
    ...
    try {
      await apiUpdateTask(editingSubtask.id, { title: trimmedTitle });
    } catch (err) {
  ...
  const DroppableSidebarItem = ({ id, children, className, onClick }: {
    ...
    return (
      <div
        ref={setNodeRef}
        className={`${className} ${isOver ? 'bg-blue-100 dark:bg-blue-900 border-blue-400 dark:border-blue-600 ring-2 ring-blue-300 dark:ring-blue-700 ring-opacity-50' : ''} transition-all duration-200 border-2 border-transparent relative z-50`}
        onClick={onClick}
        style={{ 
          minHeight: '40px',
          pointerEvents: 'auto',
      ...
      >
        {children}
        {isOver && (
          <div className="absolute inset-0 bg-blue-200 dark:bg-blue-800 opacity-20 rounded-md pointer-events-none" />
        )}
      </div>
    );
  };
                ...
                <div className="flex items-center space-x-3 px-4 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-xs text-gray-600 dark:text-gray-400">
                      {pomodoroTimer.isBreak ? 'Break time' : getTaskById(pomodoroTimer.taskId)?.title || 'Focus session'}
                    </div>
                  </div>
      ...
      {showTaskDetails && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 w-2xl max-w-90vw max-h-90vh overflow-y-auto">
            {(() => {
              const task = getTaskById(showTaskDetails);
              if (!task) return null;
