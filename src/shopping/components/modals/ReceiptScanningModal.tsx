...
export function ReceiptScanningModal({ isOpen, onClose, onAddToPantry, onLogExpense }: ReceiptScanningModalProps) {
  const {
    receiptImageUrl,
    receiptText,
    receiptCameraOn,
  ...
  const [receiptSelectAll, setReceiptSelectAll] = React.useState(false);
  const [receiptBulkCategory, setReceiptBulkCategory] = React.useState<ShoppingItem['category']>('pantry');
  const [receiptBulkThreshold, setReceiptBulkThreshold] = React.useState('');
                      ...
                      <tr key={it.id} className={idx % 2 ? 'bg-white' : 'bg-gray-50/50'}>
                        <td className="px-3 py-2 align-top">
                          <input
                            type="checkbox"
                            checked={it.selected}
                            onChange={(e) => setParsedReceipt(list => list.map(x => x.id === it.id ? { ...x, selected: e.target.checked } : x))}
                          />
                        </td>
                        <td className="px-3 py-2 align-top">
                        ...
                        <td className="px-3 py-2 align-top">
                          <input
                            ...
                            min={1}
                            value={it.quantity}
                            onChange={(e) => setParsedReceipt(list => list.map(x => x.id === it.id ? { ...x, quantity: Math.max(1, Number(e.target.value)||1) } : x))}
                        ...
                        <td className="px-3 py-2 align-top">
                          <input
                            ...
                            min={0}
                            value={it.threshold}
                            onChange={(e) => setParsedReceipt(list => list.map(x => x.id === it.id ? { ...x, threshold: e.target.value } : x))}
                ...
                <div className="border-t p-4 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-gray-900">
                      ${(receiptMeta.total != null ? receiptMeta.total : receiptCategorySummary.estSubtotal).toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
