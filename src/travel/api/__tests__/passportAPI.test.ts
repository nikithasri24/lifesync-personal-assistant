...
describe('passportAPI', () => {
  ...
  describe('getUserPassports', () => {
    it('should fetch all passports for authenticated user', async () => {
      const mockPassports = [
        { id: 'passport-1', user_id: 'test-user-123', country_code: 'US', country_name: 'United States',
          passport_number: 'US123456', issue_date: '2020-01-01', expiry_date: '2030-01-01', is_primary: true,
          created_at: '2025-11-19T12:00:00Z', updated_at: '2025-11-19T12:00:00Z' },
        { id: 'passport-2', user_id: 'test-user-123', country_code: 'GB', country_name: 'United Kingdom',
          passport_number: 'GB789012', issue_date: '2021-01-01', expiry_date: '2031-01-01', is_primary: false,
          created_at: '2025-11-18T12:00:00Z', updated_at: '2025-11-18T12:00:00Z' },
      ];
      const mockQuery = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        order: vi.fn().mockReturnThis().mockReturnValue({
          order: vi.fn().mockResolvedValue({ data: mockPassports, error: null }),
        }),
      };
      const { from: fromMock } = supabase as any;
  ...
  describe('getPrimaryPassport', () => {
    it('should fetch primary passport for authenticated user', async () => {
      const mockPassport = { id: 'passport-1', user_id: 'test-user-123', country_code: 'US',
        country_name: 'United States', passport_number: 'US123456', issue_date: '2020-01-01',
        expiry_date: '2030-01-01', is_primary: true, created_at: '2025-11-19T12:00:00Z',
        updated_at: '2025-11-19T12:00:00Z' };
      const mockQuery = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: mockPassport, error: null }),
      };
      (supabase.from as any).mockReturnValue(mockQuery);
    ...
    it('should throw error for non-PGRST116 errors', async () => {
      const mockQuery = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: null, error: { message: 'Other database error' } }),
      };
      (supabase.from as any).mockReturnValue(mockQuery);
  ...
  describe('addPassport', () => {
    it('should create a new passport', async () => {
      const mockInsertedPassport = { id: 'passport-new', user_id: 'test-user-123', country_code: 'CA',
        country_name: 'Canada', passport_number: 'CA123456', issue_date: '2022-01-01',
        expiry_date: '2032-01-01', is_primary: false, created_at: '2025-11-19T12:00:00Z',
        updated_at: '2025-11-19T12:00:00Z' };
      const mockInsertQuery = {
        insert: vi.fn().mockReturnThis(),
        select: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: mockInsertedPassport, error: null }),
      };
      const { from: fromMock } = supabase as any;
      fromMock.mockReturnValue(mockInsertQuery);
      const newPassport = { countryCode: 'CA', countryName: 'Canada', passportNumber: 'CA123456',
        issueDate: '2022-01-01', expiryDate: '2032-01-01', isPrimary: false };
      const result = await addPassport(newPassport);
      expect(fromMock).toHaveBeenCalledWith('user_passports');
      expect(mockInsertQuery.insert).toHaveBeenCalledWith(
        expect.objectContaining({
          user_id: mockUser.id, country_code: 'CA', country_name: 'Canada',
          passport_number: 'CA123456', issue_date: '2022-01-01', expiry_date: '2032-01-01',
          is_primary: false,
        })
      );
    ...
    it('should unset other primary passports when adding a new primary passport', async () => {
      ...
      const mockUpdateQuery = { update: vi.fn().mockReturnValue({ eq: mockUpdateEq1 }) };
      ...
      (supabase.from as any).mockReturnValueOnce(mockUpdateQuery).mockReturnValueOnce(mockInsertQuery);
      await addPassport({ countryCode: 'CA', countryName: 'Canada', isPrimary: true });
    ...
    it('should throw error when not authenticated', async () => {
      (supabase.auth.getUser as any).mockResolvedValue({ data: { user: null } });
      await expect(addPassport({ countryCode: 'CA', countryName: 'Canada', isPrimary: false }))
  ...
  describe('updatePassport', () => {
    it('should update an existing passport', async () => {
      const mockUpdatedPassport = { id: 'passport-1', user_id: 'test-user-123', country_code: 'US',
        country_name: 'United States', passport_number: 'US-UPDATED', issue_date: '2020-01-01',
        expiry_date: '2030-01-01', is_primary: true, created_at: '2025-11-19T12:00:00Z',
      ...
      const mockQuery = {
        ...
        eq: vi.fn().mockReturnThis(),
        select: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: mockUpdatedPassport, error: null }),
      };
      (supabase.from as any).mockReturnValue(mockQuery);
      const result = await updatePassport('passport-1', { passportNumber: 'US-UPDATED' });
      expect(mockQuery.update).toHaveBeenCalledWith(
        expect.objectContaining({ passport_number: 'US-UPDATED' })
      );
    ...
    it('should unset other primary passports when setting as primary', async () => {
      ...
      const mockUpdatePrimaryQuery = { update: vi.fn().mockReturnValue({ eq: mockPrimaryEq1 }) };
      ...
      const mockUpdateQuery = { update: vi.fn().mockReturnValue({ eq: mockEq1 }) };
      mockEq1.mockReturnValue({ eq: mockEq2 });
      mockEq2.mockReturnValue({ select: mockSelect });
      mockSelect.mockReturnValue({ single: mockSingle });
      (supabase.from as any).mockReturnValueOnce(mockUpdatePrimaryQuery).mockReturnValueOnce(mockUpdateQuery);
    ...
    it('should only update provided fields', async () => {
      ...
      const mockQuery = { update: vi.fn().mockReturnValue({ eq: mockEq1 }) };
      mockEq1.mockReturnValue({ eq: mockEq2 });
      mockEq2.mockReturnValue({ select: mockSelect });
      mockSelect.mockReturnValue({ single: mockSingle });
      (supabase.from as any).mockReturnValue(mockQuery);
    ...
    it('should throw error when not authenticated', async () => {
      (supabase.auth.getUser as any).mockResolvedValue({ data: { user: null } });
      await expect(updatePassport('passport-1', { passportNumber: 'NEW' }))
        .rejects.toThrow('Not authenticated');
    });
    ...
    it('should throw error when update fails', async () => {
      ...
      (supabase.from as any).mockReturnValue(mockQuery);
      await expect(updatePassport('passport-1', { passportNumber: 'NEW' })).rejects.toThrow();
    });
  });
  ...
  describe('getUserVisas', () => {
    it('should fetch all visas for authenticated user', async () => {
      const mockVisas = [
        { id: 'visa-1', user_id: 'test-user-123', country_code: 'JP', country_name: 'Japan',
          visa_type: 'Tourist', issue_date: '2025-01-01', expiry_date: '2026-01-01',
          multiple_entry: true, max_stay_days: 90, notes: 'Test visa',
          created_at: '2025-11-19T12:00:00Z', updated_at: '2025-11-19T12:00:00Z' },
        { id: 'visa-2', user_id: 'test-user-123', country_code: 'CN', country_name: 'China',
          visa_type: 'Business', issue_date: '2024-06-01', expiry_date: '2024-12-01',
          multiple_entry: false, max_stay_days: 30, notes: null,
          created_at: '2025-11-18T12:00:00Z', updated_at: '2025-11-18T12:00:00Z' },
      ];
      const mockQuery = {
        select: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        order: vi.fn().mockResolvedValue({ data: mockVisas, error: null }),
      };
      const { from: fromMock } = supabase as any;
      fromMock.mockReturnValue(mockQuery);
  ...
  describe('getActiveVisas', () => {
    it('should fetch only active (non-expired) visas', async () => {
      const mockVisas = [
        { id: 'visa-1', user_id: 'test-user-123', country_code: 'JP', country_name: 'Japan',
          visa_type: 'Tourist', issue_date: '2025-01-01', expiry_date: '2026-01-01',
          multiple_entry: true, max_stay_days: 90, notes: null,
          created_at: '2025-11-19T12:00:00Z', updated_at: '2025-11-19T12:00:00Z' },
      ];
      const mockQuery = {
        select: vi.fn().mockReturnThis(),
  ...
  describe('addVisa', () => {
    it('should create a new visa', async () => {
      const mockInsertedVisa = { id: 'visa-new', user_id: 'test-user-123', country_code: 'IN',
        country_name: 'India', visa_type: 'Tourist', issue_date: '2025-01-01',
        expiry_date: '2026-01-01', multiple_entry: true, max_stay_days: 90, notes: 'E-visa',
        created_at: '2025-11-19T12:00:00Z', updated_at: '2025-11-19T12:00:00Z' };
      const mockQuery = {
        insert: vi.fn().mockReturnThis(),
        select: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: mockInsertedVisa, error: null }),
      };
      const { from: fromMock } = supabase as any;
      fromMock.mockReturnValue(mockQuery);
      const newVisa = { countryCode: 'IN', countryName: 'India', visaType: 'Tourist',
        issueDate: '2025-01-01', expiryDate: '2026-01-01', multipleEntry: true,
        maxStayDays: 90, notes: 'E-visa' };
      const result = await addVisa(newVisa);
      expect(fromMock).toHaveBeenCalledWith('user_visas');
      expect(mockQuery.insert).toHaveBeenCalledWith(
        expect.objectContaining({
          user_id: mockUser.id, country_code: 'IN', country_name: 'India',
          visa_type: 'Tourist', issue_date: '2025-01-01', expiry_date: '2026-01-01',
          multiple_entry: true, max_stay_days: 90, notes: 'E-visa',
        })
      );
    ...
    it('should handle optional fields', async () => {
      ...
      (supabase.from as any).mockReturnValue(mockQuery);
      const newVisa = { countryCode: 'BR', countryName: 'Brazil', expiryDate: '2026-06-01',
        multipleEntry: false };
    ...
    it('should throw error when not authenticated', async () => {
      (supabase.auth.getUser as any).mockResolvedValue({ data: { user: null } });
      await expect(
        addVisa({ countryCode: 'IN', countryName: 'India', expiryDate: '2026-01-01', multipleEntry: true })
      ).rejects.toThrow('Not authenticated');
    });
  ...
  describe('updateVisa', () => {
    it('should update an existing visa', async () => {
      const mockUpdatedVisa = { id: 'visa-1', user_id: 'test-user-123', country_code: 'JP',
        country_name: 'Japan', visa_type: 'Tourist', issue_date: '2025-01-01',
        expiry_date: '2027-01-01', multiple_entry: true, max_stay_days: 90, notes: 'Extended',
        created_at: '2025-11-19T12:00:00Z', updated_at: '2025-11-19T13:00:00Z' };
      const mockQuery = {
        update: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        select: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: mockUpdatedVisa, error: null }),
      };
      (supabase.from as any).mockReturnValue(mockQuery);
      const result = await updateVisa('visa-1', { expiryDate: '2027-01-01', notes: 'Extended' });
      expect(mockQuery.update).toHaveBeenCalledWith(
        expect.objectContaining({ expiry_date: '2027-01-01', notes: 'Extended' })
      );
    ...
    it('should throw error when not authenticated', async () => {
      (supabase.auth.getUser as any).mockResolvedValue({ data: { user: null } });
      await expect(updateVisa('visa-1', { notes: 'Updated' })).rejects.toThrow('Not authenticated');
    });
    ...
    it('should throw error when update fails', async () => {
      ...
      (supabase.from as any).mockReturnValue(mockQuery);
      await expect(updateVisa('visa-1', { notes: 'Updated' })).rejects.toThrow();
    });
  });
