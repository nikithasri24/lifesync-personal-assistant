      ...
      <ol className="space-y-1.5">
        {products.map((product, idx) => (
          <li key={product.id} className="flex items-start gap-1.5 text-xs">
            <span className="text-gray-400 font-medium min-w-[16px] text-[10px]">{idx + 1}.</span>
            <div className="flex-1 min-w-0">
              <p className="text-gray-900 font-medium truncate" title={product.name}>
                {product.name}
              </p>
              {product.brand && (
                <p className="text-[10px] text-gray-500 truncate" title={product.brand}>
                  {product.brand}
                </p>
              )}
            </div>
    ...
    <div className={`bg-white rounded-xl border border-gray-200 shadow-sm ${className}`}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-900">Weekly Skincare Schedule</h2>
        </div>
      ...
      <div className="overflow-x-auto">
        ...
        <div className="grid" style={{ gridTemplateColumns: '140px repeat(2, minmax(200px, 1fr))' }}>
          <div className="p-3 border-b border-r border-gray-200 bg-gray-50 sticky left-0 z-20" />
          {TIME_SLOTS.map((timeSlot) => (
            <div
              key={timeSlot}
              className="p-3 border-b border-r border-gray-200 text-sm font-semibold text-gray-900 bg-gray-50 text-center flex items-center justify-center gap-2"
            >
              {timeSlot === 'AM' ? (
                <>
        ...
        {weekDates.map((date, index) => {
          ...
          return (
            <div key={index} className="grid" style={{ gridTemplateColumns: '140px repeat(2, minmax(200px, 1fr))' }}>
              {/* Day label */}
              <div
                className={`relative p-3 border-b border-r border-gray-200 bg-gray-50 text-sm font-medium flex flex-col justify-center sticky left-0 z-10 ${
                  isToday ? 'text-emerald-700 font-semibold' : 'text-gray-800'
                }`}
              >
                {isToday && (
              ...
              {TIME_SLOTS.map((timeSlot) => {
                ...
                return (
                  <div
                    key={`${index}-${timeSlot}`}
                    className={`group relative p-3 border-b border-r border-gray-200 min-h-[120px] ${
                      hasContent
                        ? timeSlot === 'AM'
                          ? 'bg-amber-50/30'
                          : 'bg-indigo-50/30'
                        : 'bg-white'
                    } ${isToday ? 'border-l-2 border-l-emerald-300' : ''}`}
                  >
                    {/* Action buttons */}
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {routinesForSlot.length > 0 && (
                        <button
                          ...
                          className="p-1 rounded bg-white hover:bg-gray-100 border border-gray-200 shadow-sm"
                          title="Edit routine"
                        >
                      ...
                      >
                        <Plus className={`h-3 w-3 ${timeSlot === 'AM' ? 'text-amber-700' : 'text-indigo-700'}`} />
                      </button>
                    </div>
