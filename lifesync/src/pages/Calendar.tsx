                    ...
                    <div key={day.toISOString()} className="flex-1 relative border-r border-gray-200 last:border-r-0">
                      ...
                      {getSpanningEventsForDay(day).map((event) => {
                        ...
                        return (
                          <div
                            key={event.id}
                            className={`absolute left-1 right-1 rounded-md shadow-sm border border-opacity-30 cursor-move hover:shadow-md transition-shadow z-10 group ${
                              isBeingDragged || isBeingResized ? 'opacity-75 scale-105' : ''
                            }`}
                            style={{
                              backgroundColor: event.color,
                              borderColor: event.color,
                              top: `${displayEvent.startHour * 64 + displayEvent.topOffset}px`,
                              height: `${Math.max(20, displayEvent.height - 2)}px`,
                            }}
                            onMouseDown={(e) => handleEventMouseDown(event, e)}
                          >
                            {/* Resize handle - top */}
                            <div className="absolute top-0 left-0 right-0 h-2 cursor-ns-resize opacity-0 group-hover:opacity-100 bg-white bg-opacity-20 transition-opacity" />
                            
                            <div className="p-1 text-white text-xs font-medium h-full flex flex-col justify-start pointer-events-none">
                              <div className="truncate leading-tight">{displayEvent.title}</div>
                              {displayEvent.height > 40 && (
                                <div className="text-xs opacity-90 mt-0.5">
                                  {format(new Date(displayEvent.startTime), 'h:mm a')} - {format(new Date(displayEvent.endTime), 'h:mm a')}
                                </div>
                              )}
                              {displayEvent.height > 60 && event.location && (
                                <div className="text-xs opacity-75 mt-0.5 truncate">📍 {event.location}</div>
                              )}
                            </div>

                            {/* Resize handle - bottom */}
                            <div className="absolute bottom-0 left-0 right-0 h-2 cursor-ns-resize opacity-0 group-hover:opacity-100 bg-white bg-opacity-20 transition-opacity" />
                          </div>
                        );
                      })}
                      ...
                      {eventDragPreview && draggedEvent && isSameDay(eventDragPreview.day, day) && !getSpanningEventsForDay(day).find(e => e.id === draggedEvent.id) && (
                        <div
                          className="absolute left-1 right-1 rounded-md border-2 border-dashed border-opacity-60 z-20 pointer-events-none"
                          style={{
                            borderColor: draggedEvent.color,
                            backgroundColor: `${draggedEvent.color}40`, // 25% opacity
                            top: `${eventDragPreview.startTime.getHours() * 64 + (eventDragPreview.startTime.getMinutes() / 60) * 64}px`,
                            height: `${Math.max(20, ((eventDragPreview.endTime.getTime() - eventDragPreview.startTime.getTime()) / (1000 * 60 * 60)) * 64 - 2)}px`,
                          }}
                        >
                          <div className="p-1 text-gray-700 text-xs font-medium h-full flex flex-col justify-start">
                            <div className="truncate leading-tight">{draggedEvent.title}</div>
                            <div className="text-xs opacity-90 mt-0.5">
              ...
              <div className="flex-1 flex">
                ...
                {getSpanningEventsForDay(currentDate).map((event) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className={`absolute left-2 right-2 rounded-md shadow-sm border border-opacity-30 cursor-move hover:shadow-md transition-shadow z-10 group ${
                        isBeingDragged || isBeingResized ? 'opacity-75 scale-105' : ''
                      }`}
                      style={{
                        backgroundColor: event.color,
                        borderColor: event.color,
                        top: `${displayEvent.startHour * 64 + displayEvent.topOffset}px`,
                        height: `${Math.max(24, displayEvent.height - 2)}px`,
                      }}
                      onMouseDown={(e) => handleEventMouseDown(event, e)}
                    >
                      {/* Resize handle - top */}
                      ...
                      <div className="p-2 text-white text-sm font-medium h-full flex flex-col justify-start pointer-events-none">
                        <div className="font-semibold leading-tight">{displayEvent.title}</div>
